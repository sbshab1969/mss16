...
public class VarManagerEditSqlJpa extends ManagerBaseHiber implements IVarManagerEdit {
  private static Logger logger = LoggerFactory.getLogger(VarManagerEditSqlJpa.class);
...
//    sbQuery.append("select {t.*} from mss_vars t");
    ...
    EntityTransaction tx = dbConnect.getEntityTransaction(entityManager);
    try {
      Query query = entityManager.createNativeQuery(strQuery, VarClass.class);
//          .addEntity("t",VarClass.class);
      objList = (List<VarClass>) query.getResultList();
      tx.commit();
    } catch (Exception e) {
      tx.rollback();
      DialogUtils.errorPrint(e,logger);
