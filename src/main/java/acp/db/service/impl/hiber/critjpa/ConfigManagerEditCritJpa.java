...
public class ConfigManagerEditCritJpa extends ManagerBaseHiber implements IConfigManagerEdit {
  private static Logger logger = LoggerFactory.getLogger(ConfigManagerEditCritJpa.class);
  ...
  public String getCfgName(Long objId) {
    ...
    try {
      ...
      criteria.select(pName);
      criteria.where(builder.equal(pId,objId));
      // --------------------
      configName = entityManager.createQuery(criteria).getSingleResult();
      // --------------------
      tx.commit();
    } catch (Exception e) {
      tx.rollback();
  ...
  public String getCfgStr(Long objId) {
    ...
    try {
      ...
      Root<ConfigClass> root = criteria.from(ConfigClass.class);
      Path<Long> pId = root.get("id"); 
      Path<String> pName = root.get("configR"); 
      criteria.select(pName);
      criteria.where(builder.equal(pId,objId));
      // --------------------
      configStr = entityManager.createQuery(criteria).getSingleResult();
      // --------------------
      tx.commit();
    } catch (Exception e) {
      tx.rollback();
